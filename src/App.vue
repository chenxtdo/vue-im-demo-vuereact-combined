    ...
    <Provider v-if="!!nimKitCore" :nimKitCore="nimKitCore" :initOptions="initOptions" >
      ...
      <div :class="$style.content">
        ...
        <div :class="$style.right" v-if="model === 'contact'">
          ...
          <div :class="$style['right-content']">
            <ContactInfoContainer :afterSendMsgClick="() => model = 'chat'" :onGroupItemClick="() => model = 'chat'" />
          </div>
        </div>
      </div>
    </Provider>
...
export default {
  ...
  mounted: function () {
    ...
    const nimKitCore = new NIM({ initOptions: this.initOptions })
    this.nimKitCore = nimKitCore
    nimKitCore.on('logined', (res) => {
